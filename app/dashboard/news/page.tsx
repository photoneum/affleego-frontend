...
export default function NewsPage() {
  ...
  // Memphis design background elements with news theme
  const MemphisBackground = () => (
    <div className="absolute inset-0 overflow-hidden rounded-xl">
      {/* Geometric shapes with news theme colors */}
      <motion.div
        className="absolute -right-4 -top-4 size-16 rotate-45 bg-gradient-to-br from-cyan-400 to-blue-600 opacity-20 sm:size-20 md:size-24"
        animate={{ rotate: [45, 65, 45] }}
        transition={{ duration: 4, repeat: Infinity, ease: "linear" }}
      />
      <motion.div
        className="absolute bottom-6 left-6 size-12 rotate-12 rounded-full bg-gradient-to-r from-purple-400 to-indigo-500 opacity-25 sm:size-14 md:size-16"
        animate={{ scale: [1, 1.2, 1] }}
        transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute right-8 top-1/2 size-8 bg-gradient-to-l from-teal-400 to-cyan-500 opacity-20 sm:size-10 md:size-12"
        animate={{ rotate: [0, 360] }}
        transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
      />
      <div className="absolute bottom-1/4 right-1/4 h-6 w-16 rotate-[30deg] rounded-full bg-gradient-to-r from-blue-400 to-purple-500 opacity-15 sm:h-8 sm:w-20 md:h-10 md:w-24" />

      {/* News-themed decorative elements */}
      <motion.div
        className="absolute left-8 top-1/4 size-6 rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 opacity-30 sm:size-8 md:size-10"
      ...
      <div className="absolute bottom-1/3 right-6 h-px w-8 -rotate-12 bg-gradient-to-r from-transparent via-cyan-300 to-transparent opacity-25 sm:w-12 md:w-16" />
    </div>
  );
  ...
  const features = [
    {
      ...
      title: "Industry News",
      description:
        "Latest updates from the affiliate marketing world and industry trends",
    },
    {
      icon: TrendingUp,
      title: "Market Insights",
      description: "Real-time analytics and performance data from top networks",
    },
    {
      icon: Zap,
      title: "Instant Alerts",
      description:
        "Breaking news and urgent updates delivered straight to your dashboard",
    },
  ];
    ...
    >
      <motion.div variants={cardMotion}>
        <DashboardPageHeader
          ...
          description="Stay ahead with the latest industry updates"
        />
      </motion.div>
      ...
      <motion.div
        ...
        variants={cardMotion}
      >
        <Card className="relative w-full max-w-2xl overflow-hidden border-0 bg-gradient-to-br from-blue-500 via-cyan-400 to-purple-500 text-white shadow-2xl">
          <MemphisBackground />
          <CardContent className="relative z-10 flex flex-col items-center px-6 py-8 sm:px-8 sm:py-12 md:px-12 md:py-16">
            ...
            >
              <MessageSquare size={48} className="sm:size-16" />
            </motion.div>
            ...
            >
              ...
              exclusive alerts that matter to your business success.
            </motion.p>
          ...
          >
            <Card className="border-gray-800 bg-gray-900/50 backdrop-blur-sm transition-all duration-300 hover:border-gray-700 hover:bg-gray-900/70">
              <CardContent className="p-4 sm:p-6">
                <motion.div
                  className="mb-3 inline-flex rounded-lg bg-gradient-to-br from-blue-400/20 to-purple-500/20 p-2 sm:mb-4 sm:p-3"
                  whileHover={{ scale: 1.1 }}
                  transition={{ type: "spring", stiffness: 400, damping: 17 }}
                >
                  <feature.icon className="size-5 text-cyan-400 sm:size-6" />
                </motion.div>
      ...
      >
        ...
        >
          {[
            { icon: BellRing, label: "Breaking News", color: "text-red-400" },
            {
              icon: TrendingUp,
              label: "Market Trends",
              color: "text-green-400",
            },
            { icon: Globe, label: "Global Updates", color: "text-blue-400" },
            { icon: Zap, label: "Flash Alerts", color: "text-yellow-400" },
          ].map((category) => (
            <motion.div
              key={category.label}
              ...
              transition={{ type: "spring", stiffness: 400, damping: 17 }}
            >
              <category.icon className={`size-4 ${category.color} sm:size-5`} />
